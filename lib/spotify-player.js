...
SpotifyPlayer.prototype.searchPlaylistsAndPlay = function(query) {
    ...
    // Search playlists
    self.spotifyApi.searchPlaylists(query, { limit: 5, offset: 0 })
        .then(function(data1) {
                for (var i in data1.body.playlists.items) {
                    var playlistObj = data1.body.playlists.items[i];
                    ...
                    self.spotifyApi.getPlaylistTracks(playlistObj.owner.id, playlistObj.id, { limit: 50, offset: 0 }, (error, data2) => {
                        ...
                        if (i == (data1.body.playlists.items.length - 1)) {
